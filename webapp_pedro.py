...
def graf_linha_tempo(df=None, nome=None):
    fig = px.line(data_frame=df, x="ano", y="quantidade", color="tipo estudo", 
                    title=f'{nome} ({df["ano"].min()} - 2022): {df["quantidade"].sum()} estudos')
    fig.update_layout(
...
with tab1:
    ...
    graf_linha_tempo(df=cervicalgia_df1, nome='Cervicalgia')
...
with tab2:
    ...
    st.subheader('Quantidade de estudos')
    graf_linha_tempo(df=lombalgia_df1, nome='Lombalgia')
...
with tab3:
    ...
    st.subheader('Quantidade de estudos')
    graf_linha_tempo(df=dor_ombro_df1, nome='Dor Ombro')
